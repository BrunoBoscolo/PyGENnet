...
- `neural_network`: Contains the core logic for the neural network, including matrix operations, network creation, forward propagation, and mutation.
- `evolution`: Implements the genetic algorithm, including population creation, fitness evaluation, selection, and reproduction.
...
2.  **Build the main number recognizer application**:
    ```bash
    make recognizer
    ```
    This will create an executable named `recognizer` in the root directory.
...
### Running the Number Recognizer
The `recognizer` application is not yet implemented. The `src/number_recognizer.c` file is a stub.
...
4.  **Reproduction**: The selected parents are cloned, and their parameters (weights and biases) are slightly mutated. These new networks form the next generation.
